...
export default function DayButtonsEsp({ weekPath }) {
          ...
          {Array.from({ length: 5 }, (_, i) => i + 1).map((day) => (
            <Link
              ...
              className={`flex-1 bg-zinc-800 px-1.5 py-2 rounded text-base font-semibold text-center whitespace-nowrap border border-zinc-700 ${
                router.pathname === `${weekPath}/dia${day}` ? 'bg-zinc-700' : 'hover:bg-zinc-700'
              }`}
            >
              <span className="truncate">Día {day}</span>
            </Link>
          ))}
        </div>
      </div>
